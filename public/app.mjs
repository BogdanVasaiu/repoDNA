...
function connectSSE() {
  ...
  es.addEventListener("file_status", function (e) {
    ...
    if (window._fcRefresh) window._fcRefresh(d.file);
  });
...
function wrapPreviewSections(html) {
  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    if (fileMatch) {
      // Normalize backslashes (Windows paths) to forward slashes so the key
      // matches S.fileList ids and card.dataset.file values.
      key = fileMatch[1].replace(/\\/g, "/");
...
var _fcFindMode = "card"; // "card" or "context"
...
window._openFindCard = function () {
  ...
  overlay.innerHTML =
    '<div class="find-card-popup">' +
    '<div class="find-card-head">' +
    '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>' +
    '<input class="find-card-search" placeholder="Filter files..." autofocus />' +
    '<div class="fc-mode-toggle">' +
    '<button class="fc-mode-btn' + (_fcFindMode === "card" ? " active" : "") + '" data-mode="card" title="Navigate to result card">Card</button>' +
    '<button class="fc-mode-btn' + (_fcFindMode === "context" ? " active" : "") + '" data-mode="context" title="Find in context file preview">Context</button>' +
    '</div>' +
    '<button class="find-card-close">&times;</button>' +
    '</div>' +
    '<div class="find-card-body"></div>' +
    '</div>';

  document.body.appendChild(overlay);

  var input = overlay.querySelector(".find-card-search");
  var body = overlay.querySelector(".find-card-body");
  var closeBtn = overlay.querySelector(".find-card-close");
  var modeBtns = overlay.querySelectorAll(".fc-mode-btn");

  modeBtns.forEach(function (btn) {
    btn.addEventListener("click", function () {
      _fcFindMode = btn.dataset.mode;
      modeBtns.forEach(function (b) { b.classList.toggle("active", b.dataset.mode === _fcFindMode); });
    });
  });

  function close() {
    overlay.remove();
    window._fcRefresh = null;
  }
  ...
  function renderNode(node, depth) {
    ...
    for (var key in node) {
      ...
      if (val && typeof val === "object" && val.file) {
        fileEntries.push({ name: key, data: val });
      } else {
        folders.push({ name: key, children: val });
      }
    }
    for (var fi = 0; fi < folders.length; fi++) {
      ...
      while (true) {
        ...
        var subfolders = ckeys.filter(function (k) { return typeof children[k] === "object" && !children[k].file; });
        if (ckeys.length === 1 && subfolders.length === 1) {
          label += "/" + ckeys[0];
          children = children[ckeys[0]];
        } else break;
      }
      var pad = depth * 12;
      html += '<div class="fc-folder open">' +
        '<div class="fc-folder-head" title="' + escHtml(label) + '" style="padding-left:' + (16 + pad) + 'px">' +
        '<span class="fc-folder-arrow">&#9654;</span>' +
        '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--t4)" stroke-width="1.8"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>' +
        '<span class="fc-folder-name">' + escHtml(label) + '</span>' +
        '</div>' +
        '<div class="fc-folder-children">' + renderNode(children, depth + 1) + '</div>' +
        '</div>';
    }
    for (var fj = 0; fj < fileEntries.length; fj++) {
      var fe = fileEntries[fj];
      var ext = (fe.name.match(/(\.[^.]+)$/) || [""])[0];
      var hasResult = !!fe.data.cat;
      var catLabel = hasResult
        ? (CAT_LABELS[fe.data.cat] ? fe.data.cat.replace(/-/g, " ") : fe.data.cat)
        : "pending";
      var st = (S.fileStatuses[fe.data.file] || {}).status || "pending";
      var stateCls = hasResult ? "" : " fc-file-disabled";
      var stateAttr = hasResult ? "" : ' aria-disabled="true"';
      var pad2 = depth * 12;
      html += '<div class="fc-file' + stateCls + '" data-file="' + escHtml(fe.data.file) + '" data-cat="' + escHtml(fe.data.cat || "") + '" data-state="' + escHtml(st) + '"' + stateAttr + ' title="' + escHtml(fe.data.file) + (hasResult ? '' : ' — not yet analysed') + '" style="padding-left:' + (28 + pad2) + 'px">' +
        getFileBadge(ext) +
        '<span>' + escHtml(fe.name) + '</span>' +
        '<span class="fc-cat-badge">' + escHtml(catLabel) + '</span>' +
        '</div>';
    }
    return html;
  }

  // Scroll so the target lands `topOffset` px below the scroll viewport's top,
  // then wait for the smooth scroll to settle before triggering the highlight.
  function _scrollAndHighlight(container, target, highlightEl, topOffset) {
    var offset = typeof topOffset === "number" ? topOffset : 12;
    // Compute the scroll delta via bounding rects rather than offsetTop —
    // result-card has no positioned ancestor, so offsetTop is measured from
    // <body>, not from #results-content, and produced wildly wrong scrolls.
    var cRect = container.getBoundingClientRect();
    var tRect = target.getBoundingClientRect();
    var targetTop = container.scrollTop + (tRect.top - cRect.top) - offset;
    ...
    var poll = setInterval(function () {
      ...
      if (Math.abs(cur - lastTop) < 1) stableTicks++;
      else stableTicks = 0;
      lastTop = cur;
      if (stableTicks >= 2 || elapsed >= maxWait) {
        clearInterval(poll);
        highlightEl.classList.remove("fc-highlight");
        // Force reflow so re-adding the class restarts the animation
        void highlightEl.offsetWidth;
        highlightEl.classList.add("fc-highlight");
        setTimeout(function () { highlightEl.classList.remove("fc-highlight"); }, 5200);
      }
    }, step);
  }

  function navigateToCard(fid, cat) {
    if (S.activeTabIsPreview || S.activeTab !== cat) {
      S.activeTab = cat;
      S.activeTabIsPreview = false;
      renderTabs();
      showResultsPanel();
    ...
    setTimeout(function () {
      var card = document.querySelector('.result-card[data-file="' + CSS.escape(fid) + '"]');
      if (card) {
        var container = document.getElementById("results-content");
        _scrollAndHighlight(container, card, card);
      }
    }, 50);
  }
  ...
  function navigateToContext(fid) {
    S.activeTabIsPreview = true;
    renderTabs();
    showPreviewPanel();
    renderPreview();
    setTimeout(function () {
      var panel = document.getElementById("preview-panel");
      var normFid = _normPath(fid);
      // Match strictly by the file-path data-key. We deliberately do NOT
      // fall back to a broad textContent search — the dependency graph and
      // file tree sections mention many file paths and would steal the match.
      var target = null;
      var els = panel.querySelectorAll(".preview-section[data-key]");
      for (var i = 0; i < els.length; i++) {
        if (_normPath(els[i].getAttribute("data-key")) === normFid) { target = els[i]; break; }
      }
      if (target) {
        _scrollAndHighlight(panel, target, target);
      } else {
        showSnack("This file isn't in the context file yet.", "warn", 3500);
      }
    }, 80);
  }

  function _currentFilter() { return (input && input.value) || ""; }

  function render(filter) {
    // Build a fid -> category map so we know which files have results ready.
    var catByFid = {};
    for (var cat in S.results) {
      var items = S.results[cat] || [];
      for (var i = 0; i < items.length; i++) {
    ...
    var filtered = filter
      ? currentFiles.filter(function (f) { return f.file.toLowerCase().indexOf(filter.toLowerCase()) !== -1; })
      : currentFiles;
    if (!filtered.length) {
      body.innerHTML = '<div class="find-card-empty">No matching files</div>';
      return;
    }
    var tree = buildTree(filtered);
    body.innerHTML = renderNode(tree, 0);

    body.querySelectorAll(".fc-folder-head").forEach(function (h) {
      h.addEventListener("click", function () {
        h.parentElement.classList.toggle("open");
      });
    });
    body.querySelectorAll(".fc-file").forEach(function (f) {
      f.addEventListener("click", function () {
        if (f.classList.contains("fc-file-disabled")) return;
        var fid = f.dataset.file;
        var cat = f.dataset.cat;
        close();
        if (_fcFindMode === "context") {
          navigateToContext(fid);
        } else {
          navigateToCard(fid, cat);
        }
      });
    });
  }

  // Live updates: when a file finishes (or its status changes),
  // upgrade just that row in place — no full re-render, no flicker.
  window._fcRefresh = function (changedFid) {
    if (!overlay.parentNode) { window._fcRefresh = null; return; }
    if (changedFid) {
      var row = body.querySelector('.fc-file[data-file="' + CSS.escape(changedFid) + '"]');
      if (row) {
        var st = (S.fileStatuses[changedFid] || {}).status || "pending";
        var cat = null;
        for (var c in S.results) {
          var arr = S.results[c] || [];
          for (var i = 0; i < arr.length; i++) {
            if (arr[i].file === changedFid) { cat = c; break; }
          }
          if (cat) break;
        }
        if (cat) {
          row.classList.remove("fc-file-disabled");
          row.removeAttribute("aria-disabled");
          row.dataset.cat = cat;
          row.title = changedFid;
          var badge = row.querySelector(".fc-cat-badge");
          if (badge) badge.textContent = CAT_LABELS[cat] ? cat.replace(/-/g, " ") : cat;
        }
        row.dataset.state = st;
        return;
      }
    }
    // Fallback: file isn't in the tree yet (new file added mid-run) — full render.
    render(_currentFilter());
  };
